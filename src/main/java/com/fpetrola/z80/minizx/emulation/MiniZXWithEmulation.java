...
public class MiniZXWithEmulation {
  ...
  protected void loadSnapshotFromBase64(final State<WordNumber> state, String base64Z80File) {
    try {
      SpectrumState snapState = getSpectrumStateFromBase64(base64Z80File);
  ...
  private void loadIntoMemory(State<WordNumber> state, SpectrumState snapState) {
    ...
    position = copyPage(ram, 5, position, memory.getData());
    position = copyPage(ram, 2, position, memory.getData());
    copyPage(ram, 0, position, memory.getData());
  }
  ...
  protected void main() {
    ...
    state = new State(new MockedIO(), new DefaultRegisterBankFactory().createBank(), new MockedMemory());
