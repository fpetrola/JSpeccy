...
public final class PixelView extends JComponent {
  ...
  /**
   * The menu creator is used to create popup menus when the user right-clicks on
   * the hex view control.
   */
  ...
  /**
   * Timer that is used to refresh the component if no data for the selected range
   * is available.
   */
  ...
  private int counter;
  boolean[] bitsWritten = new boolean[65536 * 8];
  ...
  /**
   * Moves the current position of the caret and notifies the listeners about the
   * position change.
   *
  ...
  /**
   * Updates the maximum scroll range of the scroll bar depending on the number of
   * bytes in the current data set.
   */
...
//		}
  ...
  private BufferedImage createPixelImage(final int border, final int gridSize, Color color) {
    ...
    falseGraphics.fillRect(border, border, gridSize - (border * 1), gridSize - (border * 1));
    return falseBit;
  }
...
//    }
  ...
  /**
   * Returns a a flag that indicates whether the bytes inside a column are flipped
   * or not.
   *
  ...
  /**
   * Changes the definition status of the JHexView component. This flag determines
   ...
   *
   * @param creator The new menu creator. If this parameter is null, no context
