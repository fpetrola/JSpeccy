...
public final class PixelView extends JComponent {
	 ...
	 */
	private static final String[] HEX_BYTES = { "00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "0A", "0B",
	...
	/**
	 * List of listeners that are notified if something happens in the hex panel
	 * component.
	 */
	...
	/**
	 * Currently selected position. Note that this field is twice as large as
	 * the length of data because nibbles can be selected.
	 */
	private long m_selectionStart = 0;
	/**
	 * Current selection length in nibbles. This value can be negative if
	 * nibbles before the current position are selected.
	 */
	...
	/**
	 * Used to store the height of a single row. This value is updated every
	 * time the component is drawn.
	 */
	private int m_rowHeight = 12;
	/**
	 * Used to store the width of a single character. This value is updated
	 * every time the component is drawn.
	 */
	...
	public PixelView() {
		...
		img = new BufferedImage(getWidth() + 1 - m_scrollbar.getWidth(),
				getHeight() + 1 - m_horizontalScrollbar.getHeight(), BufferedImage.TYPE_INT_RGB);
		...
		for (int i = 0; i < bytesToDraw; i++, currentOffset++) {
			...
			if (m_status == DefinitionStatus.DEFINED) {
				...
				if (isEnabled()) {
					...
					// Fixed: Highlighting im debugger memory window is wrong in
					// regards to the endianess selected
					final long normalizedOffset = m_flipBytes ? (currentOffset & -m_bytesPerColumn) + m_bytesPerColumn
	...
	private void drawBackground(final Graphics g) {
		...
		g.fillRect(
				-m_firstColumn * m_charWidth + m_hexViewWidth + m_offsetViewWidth, 0, m_firstColumn * m_charWidth
						+ getWidth() - (m_hexViewWidth + m_offsetViewWidth) - m_scrollbar.getWidth(),
				getHeight() - m_horizontalScrollbar.getHeight());
	...
	private void drawHexView(final Graphics g) {
		...
		for (int i = 0; i < bytesToDraw; i++, currentOffset++) {
			...
			if (m_status == DefinitionStatus.DEFINED) {
				...
				final int dataPosition = m_flipBytes
						? (i / m_bytesPerColumn) * m_bytesPerColumn + (columnBytes - (i % columnBytes) - 1) : i;
	...
	private void drawMouseOverHighlighting(final Graphics g) {
		...
		if (lastHighlightedView == Views.HEX_VIEW) {
			// If the mouse is in the hex view just one nibble must be
			// highlighted.
	...
	/**
	 * Returns the size of a hex view column in pixels (includes column
	 * spacing).
	 *
	 ...
	 *            The y coordinate.
	 *
	 * @return The nibble index at the coordinates or -1 if there is no nibble
	 *         at the coordinates.
	 */
	...
	/**
	 * Returns the index of the nibble below given coordinates in the ASCII
	 * view.
	 *
	 * @param x
	 *            The x coordinate.
	...
	/**
	 * Updates the maximum scroll range of the scroll bar depending on the
	 * number of bytes in the current data set.
	 */
...
//		// BitContainer.memBitContainers()[index];
...
//				bitsWritten[j2] = true;
//
//				// List<Integer> list= bitsUsageTimestamp.get(new
//				// SimilarTimestamp(bitContainer.timestamp()));
//				// if (list == null)
//				// bitsUsageTimestamp.put(new
//				// SimilarTimestamp(bitContainer.timestamp()), list= new
//				// ArrayList<Integer>());
//				//
//				// list.add(j2);
//				//
...
//		}
	...
	private void drawMemory(final Graphics gx) {
		...
		// if (counter++ > 0)
		{
			...
			for (int i = 0; i < 30; i++)
				offset = drawSequence(gx, bytesToDraw2, bitMapper, border, gridSize, offset, gridSize * (width * i),
						width);
		}
	}
	...
	private int drawSequence(final Graphics gx, final int bytesToDraw, BitMapper bitMapper, final int border,
			final int gridSize, final int offset, int xOffset, int width) {
		int y = 0;
		int position = 0;
		...
		for (int i = 0; i + offset < bitMapper.getSize() && i < bytesToDraw * width * 2; i++) {
			...
			//
			// int entries= 0;
			// for (Entry<SimilarTimestamp, List<Integer>> entry :
			// bitsUsageTimestamp.entrySet())
			// {
			// entries++;
	...
	private void drawPixel(final Graphics gx, final int border, int xOffset, int x, int y1, BufferedImage falsePixel,
			int position) {
	...
	public long getCurrentOffset() {
		...
		return m_flipBytes
				? (currentOffset & -m_bytesPerColumn) + m_bytesPerColumn - (currentOffset % m_bytesPerColumn) - 1
				: currentOffset;
	}
	 ...
	 *
	 * It is valid to pass null as the new data provider. This clears the
	 * display.
	 *
	 * @param data
	...
	public void setData(final IDataProvider data) {
		...
		/**
		 * Add a data listener to the new data source so that the component can
		 * be updated when the data changes.
		 */
		if (data != null) {
	 ...
	 *
	 * @param creator
	 *            The new menu creator. If this parameter is null, no context
	 *            menu is shown in the component.
	 */
	 ...
	 *
	 */
	private class InternalListener implements AdjustmentListener, MouseListener, MouseMotionListener, FocusListener,
			ICaretListener, IDataChangedListener, ComponentListener, KeyListener, MouseWheelListener {
		...
		public void mouseDragged(final MouseEvent arg0) {
			...
			if (y < m_paddingTop - (m_rowHeight - m_charHeight)) {
				...
				if (getSelectionLength() + 2 * m_bytesPerRow > 2
						* (m_dataProvider.getDataLength() - getSelectionStart())) {
					return;
				}
		...
		public void mousePressed(final MouseEvent event) {
			...
			if (event.getButton() == MouseEvent.BUTTON1 || event.getButton() == MouseEvent.BUTTON3) {
				...
				m_selectionLength = 0; // We don't want the notifiers to kick in
										// here.
				// setSelectionLength(0);
				...
				if (position != -1) {
					...
					// m_selectionLength = 0 must be notified in case the click
					// position is invalid.
	...
	/**
	 * Enumeration that is used to switch the output format of the offsets
	 * between 32 bit mode and 64 bit mode.
	 *
	 */
